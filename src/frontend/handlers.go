...
func getInventoryData(productID string) (*InventoryData, error) {
	// Here you would use the address of your inventory service
	url := fmt.Sprintf("http://inventory-service.default.svc.cluster.local:5050/inventory/%s", productID)
	...
	log.Printf("---- response before return -------: %+v", resp)
...
func (fe *frontendServer) productHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("inventory first var: %+v", inventory)

	if err != nil {
			// handle error
			renderHTTPError(log, r, w, errors.Wrap(err, "could not retrieve inventory data"), http.StatusInternalServerError)
			return
	}
...
// A function that can be used to update the inventory.
func updateInventory(productID string, quantity int) error {
  jsonData, err := json.Marshal(map[string]int{"change": -quantity})
  if err != nil {
    return err
  }
	log.Printf("---- json to send to update -------: %+v", jsonData)
  req, err := http.NewRequest("POST", fmt.Sprintf("http://inventory-service.default.svc.cluster.local:5050/inventory/update/%s", productID), bytes.NewBuffer(jsonData))
  if err != nil {
    return err
  }
  ...
  if resp.StatusCode != http.StatusOK {
    return fmt.Errorf("failed to update inventory for product %s", productID)
  }
...
func (fe *frontendServer) placeOrderHandler(w http.ResponseWriter, r *http.Request) {
	...
	// my code 2
	// Let's say you have the cart items in a variable `cartItems`
	for _, v := range order.GetOrder().GetItems() {
		log.Printf("---- inventory deletion or adding -------: %+v", int(-v.GetItem().GetQuantity()))
		err := updateInventory(v.GetItem().GetProductId(), int(-v.GetItem().GetQuantity()))
		if err != nil {
				// Handle the error appropriately.
				// You might want to log it, return an error response, etc.
